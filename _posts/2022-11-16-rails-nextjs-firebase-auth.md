...
ソースコード：https://github.com/kmgk/rails-next-firebaseauth-sample
...
Rails でやることは Next.js から送ってきた ID Token を検証すること。
...
- 公開鍵を使って、token を検証する
- token 検証失敗したら、error を起こす。
- token 検証成功したら、ユーザー情報を含めた payload 部分のデータを返す

```ruby
def get_verified_data(id_token)
  # Headerを取得するため、まず検証なしでtokenをdecodeする
...
def decode_unverified(token)
  JWT.decode(token, nil, false, { algorithm: ALGORITHM })
end
```
...
検証メソッドは二つに分けたのは、`sub`と`alg`は`JWT.decode`で自動検証できないから、別途追加検証必要。

```ruby
def verify(token, key)
  # Verify the signature and data (exclude subject and algorithm)
  begin
  ...
  # Validate subject ("sub") and algorithm ("alg")
